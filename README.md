...
running build - 516 KB of declarations, every event's input and result, every
method on `$`, every element's props. `types/claude-code.d.ts` here is the copy
v2.1.282 wrote. **It is the authority; the notes below are the map to it.**
...
`types/claude-code.d.ts` is the plugin API itself: 516 KB of declarations
...
`/plugin-types`, and the copy here is what 2.1.282 wrote. It is early access
